...
export default function MapAndListContainerComponent({ user }) {
  const supabase = createClient();
  const [userMapNotes, setUserMapNotes] = useState([]);
  const [markerPos, setMarkerPos] = useState([53.5461, -113.4938]);

  // const test = await loadUserMapNotes(user?.id);
  // console.log(test, "CONTAINER COMPONENT");
  ...
  //   READ DOCS WHY useCallback https://supabase.com/docs/guides/getting-started/tutorials/with-nextjs
  // const getMapNotes = useCallback(async () => {
  //   // console.log("hi");
  //   try {
  //     // const { data, error, status } = await supabase
  //     //   .from("map_notes")
  //     //   .select(`title, body,location`)
  //     //   .eq("user_id", user?.id);
  ...
  //     const mapNotes = await loadUserMapNotes(user?.id);

  //     console.log(mapNotes, "CONTAINER COMPONENT");
  //     setUserMapNotes(mapNotes);
  //   };
  //   loadMapNotes();
  // }, [user, supabase]);
  ...
  return (
    <div className="w-full flex h-full flex-row">
      ...
      <div className="basis-1/4 bg-neutral-950	max-h-[800px]">
        {user ? (
        ...
        <div className=" gap-4 p-4 bg-neutral-950 overflow-y-auto h-full max-h-[400px]">
          ...
          {userMapNotes?.map((note) => (
            ...
            >
              ...
              <p>{note.body}</p>
              {/* You can add more content related to the note here */}
